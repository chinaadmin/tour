define(function(require , exports ,module){
  ...
  var tool = require('model/tool');
	...
	var main = {
        index:function(){
            ...
            $('.sub-line').click(function () {
                var goodsid = $("input[type='radio']:checked").val();
                var uid = $("input[name='hiddenuid']").val();
                window.location.href=common.U('Order/selectJump',{goodsid:goodsid,uid:uid});
            });
  ...
  main.edit=function(){
      edit_validate; //编辑验证
      var html = '',j=1,k=$("#tab_3 h4").length;
      //添加天数
      $("#add_day_travel").live('click',function(){
          k++;
          j++;
          html = '<div style="margin-top: 10px"><h4 style="display:inline;">第'+k+'天</h4><div class="daydel btn green del_message" id='+k+' style="margin-left: 918px;margin-top: 10px;"><i class="icon-remove"></i></div><div>出发地-目的地<input type="text" name="start_destination[]" value="" placeholder='+"格式参照\"出发地-目的地\""+' style="margin-bottom: 5px;margin-left: 20px;"></div><div>餐饮<input type="text" name="food_beverage[]" value="" style="margin-bottom: 5px;margin-left: 78px;"></div><div>住宿<input type="text" name="accommodation[]" value="" style="margin-bottom: 5px;margin-left: 78px;"></div><script id="editor'+j+'" type="text/plain" name="introduce[]" style="width:1000px;height:300px;"></script></div>';
          $("#tab_3").append(html);
          var ue = UE.getEditor('editor'+j);
      });
      //删除天数
      $('.daydel').live('click',function() {
          var forthis = $(this);
          if($(this).context.id == k){
              require.async('base/jtDialog',function(jtDialog){
                  jtDialog.confirm(function (){
                      forthis.parent('div').remove();
                      forthis.remove();
                      k--;
                  },'确定删除吗！');
              });

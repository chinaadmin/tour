...
define(function(require,exports,module){
	...
	var ajax_list = {
        ...
        before_events:function(res,params){//渲染列表页面前的回调方法，返回false将不执行页面的渲染操作
            return true;
        },
        once_after_events:function(res,params){ //渲染列表页面后的回调方法，用于绑定页面的事件操作(只执行一次执行完后销毁)
        		return true;
        },
        ...
        request_lists:function(params,callback){//请求列表页面
            ...
            $.ajax({
                ...
                success:function(resp){
                	ajax_list.before_events(resp,params);
                    if(resp.status==common.success_code){
                        callback(resp.result,params); //渲染页面
                    }
                    ajax_list.once_after_events(resp,params);
                }
            });

			...
			for (var k = 1; k <= 7; k++) {
				...
				if (i2 < 1 || i2 > maxdays) {
					...
					if (i3 < dmins || i3 > dmaxs) {
						...
						//alert(dmins);
						//var minss=new Date(dmin).getDate();
						//var maxss=new Date(dmax).getDate();
                       // minss = 
						//alert(maxss);
						
						tmp += ' week="' + (k - 1) + '" id="' + y + '-' + m + '-' + i2 + '" title="' + ca.getl(d1,false) + ' ' + ca.getst(d1) + ' ' + ca.getsf(d1) + ' ' + ca.getlf(d1) +'"><p><em>' + i2 + '</em><em class="nl">' +  ll + '</em><br/><em class="nl acs hides" rel="100">' + '成人:' + '</em><br/><em class="nl cns hides" rel="50">' + '儿童:' + '</em>' + (function (t){if($.isFunction(showFu)){return showFu(t);}else{return ""}}(new Date(y,m-1,i2))) +'</p></td>';

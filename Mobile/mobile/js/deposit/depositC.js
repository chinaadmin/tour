...
.controller('depositC',function($scope,$rootScope,storage,depositS){
		 storage.init();
	...
	depositS.History({
		 ...
		 count:20
		   ...
		   if(res&&res.resCode=="SUCCESS"){
		   	  $scope.history = res.rechargeList;
		   	  for(var i =0;i<$scope.history.length;i++){
		   	  	 $scope.history[i].addTime = new Date($scope.history[i].addTime.replace("-", "/").replace("-", "/")).getTime();
		   	  }
...
//消费记录查询
  depositS.payRecord({
  	 ...
  	 count:20
  	  ...
  	  if(res&&res.resCode=="SUCCESS"){
  	  	 $scope.payHistory = res.payList;
  	  	 for(var i =0;i<$scope.payHistory.length;i++){
		   	  	 $scope.payHistory[i].payTime = new Date($scope.payHistory[i].payTime.replace('-','/').replace('-','/')).getTime();
		   	  }
  	  }
  })
...
$scope.timeC = new Date().Format('yyyy-MM');
var timeY = $scope.timeC.slice(0,4);
var timeM = $scope.timeC.slice(5,7);
var timeNew = timeY+'年'+timeM+'月';
$scope.timeY = timeY
$scope.timeM = timeM;
                    ...
                    if (that.hide(false, 'set') !== false) {
                        ...
                        depositS.History({
	                        	...
	                        	if(res.rechargeList.length==0){
	                        		 $scope.history =[];
	                        	}else if(res.rechargeList.length>0){
	                        		$scope.history = res.rechargeList;
	                        		for(var i =0;i<$scope.history.length;i++){
												   	  	 $scope.history[i].addTime = new Date($scope.history[i].addTime.replace('-','/').replace('-','/')).getTime();
												   	  }
                        ...
                        depositS.payRecord({
	                        	...
	                        	if(res.payList.length==0){
	                        		 $scope.payHistory =[];
	                        	}else if(res.payList.length>0){
	                        		$scope.payHistory = res.payList;
	                        		for(var i =0;i<$scope.payHistory.length;i++){
												   	  	 $scope.payHistory[i].payTime = new Date($scope.payHistory[i].payTime.replace('-','/').replace('-','/')).getTime();
												   	  }

...
.controller("detailC", function ($scope, $window, $filter, detailS, storage, ngMessage) {
	storage.init();
	...
	var token = storage.get("token") || "";
//	$scope.gid = window.location.search.replace(/[^0-9]/ig,""); //获取gid
	var gid = storage.getUrlParam('gid');
	console.log(gid)

	detailS.getInfo(function(res) {
		//console.log(res);
		$scope.hidden = true;
		$scope.detail = res.data;
		$scope.detail.price = parseFloat(res.data.price);
//      console.log($scope.detail.introduces);
        ...
        */
        ...
        for(i=0;i<$scope.detail.introduces.length;i++){
//      	console.log($scope.detail.introduces[i].introduce);
        	//while (reg.exec($scope.detail.introduces[i].introduce)) 
            //{
            //console.log(RegExp.$1);
            //var str=RegExp.$1;
            //substr(0, 5)
        	var newContent= $scope.detail.introduces[i].introduce.replace(/<img [^>]*src=['"]([^'"]+)[^>]*>/gi,function(match,capture){
            //capture,返回每个匹配的字符串
//          console.log(capture);
            if(capture.substr(0, 4)=="http"){
//	        	console.log("111")
	        	return '<img src="'+capture+'" alt="" />';
            } else{
//          	console.log("222");
                var newStr='<img src="'+adminhost+capture+'" alt="" />';
                return newStr;
            }
           });
//         console.log(newContent);
           $scope.detail.introduces[i].introduce=newContent;
            //}
            //}
        }
...
//		console.log(res.data.length);
		for (var i = 0; i < res.data.attribute_id.length; i++) {
			bannerPoint += '<div class="swiper-slide"><img src="'+res.data.attribute_id[i]+'"/></div>';
		};
		$('.swiper-container .swiper-wrapper').html(bannerPoint);
		$('.swiper-container .swiper-wrapper img').bind('error',function(){
			$(this).attr('src','../images/index/banner_picbg.png');
		});
...
//				freeMode : true,
	...
	$scope.back = function() {
		//alert(referer);
		if (referer&&!/dateChoose.html/.test(referer)) {
			storage.toPage(referer);
//			window.location.href = referer;
	...
	$scope.toChooseDate = function(){
		if(!token){
			storage.toPage('login')
		}else{
			window.location.href = "../commodity/dateChoose.html?gid="+gid;
			var href = "../commodity/detail.html?gid="+gid;
			storage.set("referer",href)
		}
	}
...
//			console.log($(window).scrollTop());
		...
		var contentH = $("#content").outerHeight(true) - 44;
//		console.log(contentH);
		if(scrollTop>contentH){
			$("#top").removeClass("a").addClass("b")
		}else{
			$("#top").removeClass("b").addClass("a")
		};
		var h2 = $(".item_2").offset().top-96; //96 = #top.height:52px + .header-box:height:44px
		var h3 = $(".item_3").offset().top-96;
		var h4 = $(".item_4").offset().top-96;
		if(scrollTop<h2){
			$(".top span").eq(0).addClass("active").siblings().removeClass("active");
		}else if((scrollTop>=h2)&&(scrollTop<h3)){
			$(".top span").eq(1).addClass("active").siblings().removeClass("active");
		}else if((scrollTop>=h3)&&(scrollTop<h4)){
			$(".top span").eq(2).addClass("active").siblings().removeClass("active");
		}else if(scrollTop>=h4){
			$(".top span").eq(3).addClass("active").siblings().removeClass("active");
		};
		//console.log(scrollTop+"==="+$scope.topshow);
	});
	//点击span标签跳转到相应位置(不可用 a标签，否则多次点击a标签后，返回的history就会记录多次，返回上个页面也要点击多次)
	$(".top span").click(function(){
//		setTimeout(function(){$(this).addClass("active").siblings().removeClass("active")},500);
		setTimeout(function(){$(this).siblings().removeClass("active")},10);
		var hr = $(this).attr("name");
//		if(hr == "#a"){
//			var anh = $(hr).offset().top-52;
// 	 	$("html,body").stop().animate({scrollTop:anh},300);
//		}else{
		    var anh = $(hr).offset().top-96;//获取到需要跳到页面地方的元素，想对于页面顶部的距离,
		    $("html,body").stop().animate({scrollTop:anh},300);//将返回值给html或body元素，让其滚动条，执行一个animate动画。
//		}
	});
	
})
.controller("huodongC", function ($scope, $window, $filter, detailS, storage, ngMessage) {
	storage.init();
	var token = storage.get("token") || "";
	var referer = storage.get("referer") || "";
	var gid = storage.getUrlParam('gid');
...
//				freeMode : true,
	...
	/*$scope.my_passenger = function() { 获取商品信息
		orderS.my_passenger({
			"token": token,
		}, function(res) {
			if(res.resCode == "SUCCESS") {
...
//		"mess": "当前未选择保险",
		...
		"check": function(v) {
			v.check = !v.check;
			if(v.check) {
				$scope.calculate.selectBoxt.push(v.id);
				$scope.total += parseFloat(v.costs) * parseInt(totalnum);
				pricesum();
			} else {
				var index = $scope.calculate.selectBoxt.indexOf(v.id);
				if(index > -1) {
					$scope.calculate.selectBoxt.splice(index, 1);
					$scope.total -= parseFloat(v.costs) * parseInt(totalnum);
					pricesum();
				}
			}
			...
			console.log($scope.calculate.selectBoxt);
		},
		"onSwitch": function() {
		}
	}
	...
	$(".top .left").click(function(){
		...
		$(".box").addClass("atLeft").removeClass("atRight");
	});
	$(".top .right").click(function(){
		$(this).addClass("active");
		$(this).siblings().removeClass("active");
		$(".box").addClass("atRight").removeClass("atLeft");
	});
	
})
.controller("dateChooseC", function ($scope,$scope, $window, $filter, detailS, storage, ngMessage) {
	$scope.priceSum=0;
	function countPrice() {
		//var list = $scope.cart;
		var n = 0;
		var kk=0;
		n=parseFloat($scope.adultPrice)*parseInt($scope.adultNum);
		n+=parseFloat($scope.childPrice)*parseInt($scope.childNum);
		/*
		for (var i = 0; i < list.length; i++) {
			if (list[i].check) {
				n += (parseInt(list[i].number) * parseFloat(list[i].goods.price));
				kk +=(parseInt(list[i].number) * parseFloat(list[i].promotions.totprice));
			};			
		};
		*/
		n = Math.round(n*100)/100;
		kk = Math.round(kk*100)/100;
		$scope.priceSum = n;
		$(".moneys").text("¥ "+$scope.priceSum);
		//console.log($scope.priceSum);
		$scope.youhuis = kk;
	};
	storage.init();
	...
	$scope.load();
      $scope.canshu = function() {
         ...
         $(".beijinss").show();
         $("#showsels").animate({bottom: "0px"}, 300 );
    };
    ...
    $scope.toPages = function() {
			...
			//alert($scope.stock);
			if(!$scope.gotime){
				ngMessage.showTip("请选择团期");
				return;
			};
			if($scope.stock<($scope.adultNum+$scope.childNum)){
				//alert($scope.stock);
				ngMessage.showTip("库存不足，请选择其他出发日期");
				return ;
			}
			storage.set("adultNum",$scope.adultNum);
			storage.set("childNum",$scope.childNum);
			storage.set("adultPrice",$scope.adultPrice);
			storage.set("childPrice",$scope.childPrice);
			storage.set("gotime",$scope.gotime);
			storage.set("gid",gid);
	...
	$scope.back = function() {
		window.location.href = "../commodity/detail.html?gid="+goods_id;
		storage.set("referer","../commodity/dateChoose.html?gid="+goods_id)
	};
	...
	$scope.add1 = function() {
		if(!$scope.adultPrice){
			ngMessage.showTip("请先选择出发日期！");
	    	return false;
	    } ;
	    if ($scope.adultNum < 10){
			$scope.adultNum++;
	    };
		countPrice()
	}
	...
	//$("#date1").width(100%);
	//alert("lll");
	$scope.showthis = function(){  //需要使用$scope.a_function() 才能显示ng-bind 和{{}} 绑定的数据；如果直接用jQuery，无法显示
	    if(!$scope.adultPrice){
	    	return false;
	    }
		$("#service").show();
		$(".consulting_service").animate({bottom:0},300);
	};
/*
	$scope.sels=function(){
        
        $(".ke_calcd").live("click",function(){
		if($(this).hasClass('has')){
			
			$(".kecalc_box tbody").find(".has").css({"background":"#fff","color":"#404040"})
			$(this).css({"background":"#00bc9f","color":"#fff"});
			$scope.adultPrice = $(this).children().eq(2).text().replace(/[^0-9]/ig,"");
			$scope.childPrice = $(this).children().eq(3).text().replace(/[^0-9]/ig,"");
			console.log($scope.adultPrice)
			if($scope.adultPrice){
				$("#adultPrice").text("¥ "+$scope.adultPrice);
                $scope.numbers= true;
        console.log($scope.numbers);
			};
			if($scope.childPrice){
				$("#childPrice").text("¥ "+$scope.childPrice);
			};
			var startTime = $(".month").html()+$(this).children().eq(1).text()+"日";
	...
	*/
	...
	$(".ke_calcd").live("click",function(){
		...
		if($(this).hasClass('has')){
			...
			$(this).css({"background":"#00bc9f","color":"#fff"});
			//$scope.adultPrice = $(this).children().eq(2).text().replace(/[^0-9]/ig,"");
			$scope.stock = $(this).children().eq(0).text().substr(1);
			$scope.adultPrice = $(this).children().eq(2).text().substr(1);
			//$scope.childPrice = $(this).children().eq(3).text().replace(/[^0-9]/ig,"");
			$scope.childPrice = $(this).children().eq(3).text().substr(1);
			console.log($scope.stock);
			//alert($(this).attr("ymd"));
			//console.log($scope.adultPrice)
			if($scope.adultPrice){
				//alert("kkk");
				//$("#childPrice").text("¥ "+$scope.adultPrice);
				$("#adultPrice").text("¥"+$scope.adultPrice);
                //$scope.numbers= true;
                //console.log($scope.numbers);
                $(".selected").show();
...
.controller('gatareaC', function($scope, $window, $filter, tool, detailS, storage, ngMessage, ngWechat, $rootScope) {
		var token = storage.get("token") || "";
		storage.init();
...
//				 alert(longitude + '|&&|' + latitude);
		...
		if($scope.city){
    		//alert("kkk");
    		//alert($scope.ByLngLat.city.name);
    		$scope.show=true;
    	}
		$scope.gatarea = [];
		detailS.getHotCity({
        ...
        $scope.load=function(){
			...
			for(i=0;i<data.data.length;i++){
				...
				for(y=0;y<data.data[i].city.length;y++){
					//console.log(data.data[i].city[y].name);
					if(data.data[i].city[y].name.indexOf(datass.search)==0){
						console.log(data.data[i].city[y].name);
						selectBox.push(data.data[i].city[y].name);
					}
				}
			}
...
var data = {
   ...
   console.log(data);
 	var hotCity = [];
    for(i=0;i<data.data.length;i++){
		for(j=0;j<data.data[i].city.length;j++){
			hotCity.push(data.data[i].city[j].name);
		}
	};
	
	function unique1(array){ //去掉重复的项
	  var n = []; //一个新的临时数组
	  //遍历当前数组
	...
	hotCity = unique1(hotCity);
	$scope.hotCity = hotCity;
	$scope.hotCity.sort(compare);

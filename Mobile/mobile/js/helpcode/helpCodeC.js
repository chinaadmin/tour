...
.controller("classificationC",function ($scope, $document,indexS,storage,ngMessage,ngWechat) {
	...
	$(function (){
...
}).controller("helpcodeC",function ($rootScope,$scope,$interval,helpcodeS,ngMessage,storage,$element,tool,ngWechat) {
		storage.init();
 	  var token = storage.get("token");
	 ...
	 //微信分享接口
	  	helpcodeS.wxsdk({ //从后台获取注入信息 并提供跳转参数 
				url: window.location.href
			}, function(res) {
				$scope.tt = JSON.stringify(res);
				if (res && res.resCode == "SUCCESS") {
...
//	  if(!token){
			//分享的code
	  	$scope.code = storage.getUrlParam("code");
	  	$scope.$applyAsync();
	  	$scope.ngDialogShow = function(){
...
//	  }
	  //助力码
//	  if(token){
//	  	helpcodeS.activityHelpCode({token:token},function(res){
//	  			$scope.code = res.code;
//	  	})
//	  }
	  var downurl;
	  $(function(){
		   $("#downloadApp").on("click", function (e) {
            ...
            if(tool.isiPhone) {
                downurl = "https://itunes.apple.com/cn/app/bi-hao-huo-shang-cheng/id1044911371?mt=8"
            }else{
            		downurl = "http://www.bihaohuo.cn/download/android/bihaohuo.apk";
            }

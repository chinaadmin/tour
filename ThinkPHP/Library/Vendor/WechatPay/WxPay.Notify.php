...
class WxPayNotify extends WxPayNotifyReply
{
	...
	public function NotifyProcess($data, &$msg){
        ...
        if(empty($is_exist)) {
            $payinfo = $payment_log_model->field(true)->where(['order_sn' => $data['out_trade_no']])->order('add_time desc')->find();
            $payment_data = [
                'update_time' => time(),
                'trade_no' => $data['transaction_id'],
                'details' => json_encode($data),
                'buyer' => $data['device_info'],
                'seller' => $data['mch_id']
            ];
            if ($payinfo['status'] == 0) {
                switch ($payinfo['order_type']) {
                    case 2://升级会员
						$result = D('Home/UserCardsale')->paying($payinfo['order_id'],$data['transaction_id'],$data['total_fee']);
						if (!$result->isSuccess()) {
							$msg = "升级失败";
							return false;
						}else{
